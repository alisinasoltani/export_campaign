...
{ selectedStyle: selectedStyle, background_colors_options: color[], font_families: fontfamily[], handleChangeBackgroundColor: any, handleFontChange: any, handleFontSizeChange: any, variant: "Button"|"Menu" }) => {
  return variant == "Menu" ? (
    ...
    <Fab component="div" className="fixed left-[2.2rem] bottom-5 shadow-lg" size="small" color="secondary" aria-label="add">
        <DropdownMenu>
            <DropdownMenuTrigger className="border-none outline-none">
                <Image src={custom_icon} width={24} alt="click to cutomize" />
            </DropdownMenuTrigger>
            <DropdownMenuContent className="flex flex-col items-start bg-transparent border-none absolute shadow-none" style={{ bottom: "2rem", left: "-1.7rem" }}>
                <DropdownMenuItem onClick={(event) => handleChangeBackgroundColor(event, ((selectedStyle.background_color_index+1) % background_colors_options.length))}>
                    <div className='flex flex-row min-w-max gap-2 justify-start items-center text-sm border-none outline-none'>
